...
#if FTS_GESTURE_EN
bool focal_gesture_mode = false;
bool focal_delay_gesture = false;
bool focal_suspend_state = false;
#define WAKEUP_OFF 4
#define WAKEUP_ON 5
...
int fts_gesture_switch(struct input_dev *dev, unsigned int type, unsigned int code, int value)
{
	...
	{
		if (focal_suspend_state)
		{
			if ((value != WAKEUP_OFF) || focal_gesture_mode)
			{
				focal_delay_gesture = true;
			}
		}
		FTS_INFO("choose the gesture mode yes or not\n");
		if(value == WAKEUP_OFF){
			FTS_INFO("disable gesture mode\n");
			focal_gesture_mode = false;
		}else if(value == WAKEUP_ON){
			FTS_INFO("enable gesture mode\n");
			focal_gesture_mode  = true;
		}
	}
	return 0;
}
...
static int fts_ts_suspend(struct device *dev)
{
	...
	if (focal_gesture_mode) {
		...
		for (i = 0; i < 5; i++) {
       		 ...
       		 fts_i2c_write_reg(ts_data->client, 0xd5, 0xff);
       		 fts_i2c_write_reg(ts_data->client, 0xd6, 0xff);
       		 fts_i2c_write_reg(ts_data->client, 0xd7, 0xff);
       		 fts_i2c_write_reg(ts_data->client, 0xd8, 0xff);
       		 fts_i2c_write_reg(ts_data->client, FTS_REG_GESTURE_EN, ENABLE);
       		 msleep(1);
       		 fts_i2c_read_reg(ts_data->client, FTS_REG_GESTURE_EN, &state);
       		 if (state == ENABLE)
          	  break;
   			 }
...
if (focal_gesture_mode){
	...
	if (focal_delay_gesture) {
		focal_gesture_mode = !focal_gesture_mode;
	}
	...
	ts_data->suspended = false;
	focal_suspend_state = false;
	focal_delay_gesture = false;
