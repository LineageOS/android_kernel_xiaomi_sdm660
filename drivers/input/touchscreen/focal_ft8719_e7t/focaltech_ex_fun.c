...
#define PROC_TP_LOCKDOWN_INFO                   "tp_lockdown_info"
...
static int fts_xiaomi_lockdown_info_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%02X%02X%02X%02X%04X%02X%02X", tp_maker_cg_lamination, display_maker, cg_ink_color, hw_version, project_id, cg_maker, reservation_byte);
...
static const struct file_operations fts_proc_lockdown_fops = 
{
	.owner = THIS_MODULE,
...
{
	ts_data->proc = proc_create(PROC_TP_LOCKDOWN_INFO, 0777, NULL, &fts_proc_lockdown_fops);
	
	if (NULL == ts_data->proc) {
        FTS_ERROR("create proc tp lockdown info note fail");
        return -ENOMEM;
    } else {
        FTS_INFO("Create proc tp lockdown info note success!");
    }
	return 0;
