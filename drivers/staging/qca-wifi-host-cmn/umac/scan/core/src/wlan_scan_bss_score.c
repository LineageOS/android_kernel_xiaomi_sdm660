...
/**
 * calculate_sae_pk_ap_weightage() - Calculate SAE-PK AP weightage
 * @entry: bss entry
 * @score_params: bss score params
 ...
 */
static uint32_t
calculate_sae_pk_ap_weightage(struct scan_cache_entry *entry,
			      struct scoring_config *score_params,
		...
		int pcl_chan_weight)
{
	...
	sae_pk_score = calculate_sae_pk_ap_weightage(entry, score_config,
						     &sae_pk_cap_present);
	score += sae_pk_score;
	...
	scm_nofl_debug("Candidate(%pM chan %d): rssi %d HT %d VHT %d HE %d su bfer %d phy %d  air time frac %d qbss %d cong_pct %d NSS %d sae_pk present %d",
	...
	scm_nofl_debug("Scores: prorated_pcnt %d rssi %d pcl %d ht %d vht %d he %d bfee %d bw %d band %d congestion %d nss %d oce wan %d sae_pk score %d TOTAL %d ",
